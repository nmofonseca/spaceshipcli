...
@respx.mock
def test_list_domains_success() -> None:
...
def test_list_domains_empty() -> None:
    """Test domain listing with no results."""
...
@respx.mock
def test_get_personal_nameservers() -> None:
    """Test getting personal nameservers."""
    domain = "example.com"
    # Mock the API response
    respx.get(
        f"https://spaceship.dev/api/v1/domains/{domain}/personal-nameservers"
    ).mock(
        return_value=Response(
            200,
            json={
...
@respx.mock
def test_get_domain_transfer() -> None:
    """Test getting domain transfer details."""
    domain = "example.com"
...
@respx.mock
def test_get_domain_auth_code() -> None:
    """Test getting domain auth code."""
    domain = "example.com"
    respx.get(f"https://spaceship.dev/api/v1/domains/{domain}/transfer/auth-code").mock(
        return_value=Response(
            200, json={"authCode": "secret123", "expires": "2024-02-01T00:00:00Z"}
        )
    )
